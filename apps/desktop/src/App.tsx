...
const englishChromeTranslations: UiChromeTranslations = {
  ...
  environment: {
    ...
    noRawClickableLink: "No raw clickable target link. Controlled browser launch requires allowlist and RealActionGate.",
    ...
    writeGateUnchanged: "Write gate unchanged: runtime fill never clicks state-changing ServiceNow controls; human review remains required.",
    configs: {
      mock: {
      ...
      qa: {
        ...
        safetyNotes: [
          ...
          "Any real QA submit requires explicit Alan approval."
        ]
      },
      dev: {
        label: "QA Test Environment",
      ...
      "production-shadow": {
        label: "Production",
        description: "Production comparison/read-only target; runtime actions stay disabled outside QA unless a separate safety review approves them.",
        safetyLabel: "No write path",
        safetyNotes: [
          "Production is visible for selection and target review, but runtime actions remain disabled outside QA.",
          "No state-changing production path is implemented.",
          "Use QA-only runtime controls for Start, Verify, and Autofill.",
...
const uiChromeTranslations: Record<LanguageCode, UiChromeTranslations> = {
  ...
  "zh-CN": {
    ...
    environment: {
      ...
      noRawClickableLink: "不显示原始可点击目标链接。受控浏览器启动需要 URL allowlist 和 RealActionGate。",
      ...
      writeGateUnchanged: "写入门禁不变：运行时填充不会点击会改变 ServiceNow 状态的控件，仍需要人工审核。",
      configs: {
        mock: {
          label: "Mock 演示",
        ...
        qa: {
          ...
          safetyNotes: ["必须手动登录。凭据绝不写入源码。", "浏览器会话保留在已忽略的本地运行目录。", "任何真实 QA 提交都需要 Alan 明确批准。"]
        },
        dev: {
          label: "QA 测试环境",
        ...
        "production-shadow": {
          label: "生产",
          description: "生产只读/对照目标；除非通过单独安全评审，否则 QA 以外的运行操作保持禁用。",
          safetyLabel: "无写入路径",
          safetyNotes: ["隐藏的非 QA 环境在这里不可用。", "没有实现会改变状态的路径。", "此工作台仅使用 QA 运行控件。", "考虑任何非 QA 写入能力前必须升级到单独安全评审。"]
        }
      }
    },
    mockForm: {
  ...
  "zh-TW": {
    ...
    environment: {
      ...
      writeGateUnchanged: "寫入門檻不變：執行時填入不會點擊會改變 ServiceNow 狀態的控制項，仍需要人工審核。"
    }
  },
  "es-ES": {
    ...englishChromeTranslations,
    ...
    environment: {
      ...
      writeGateUnchanged: "La puerta de escritura no cambia: el runtime nunca pulsa controles que cambian estado en ServiceNow; la revisión humana sigue siendo obligatoria."
    }
  }
};
...
const englishOperatorWorkbenchCopy = {
  ...
  runtime: {
    ...
    collapseRuntime: "Collapse runtime action rail",
    expandRuntime: "Expand runtime action rail",
    collapsedTitle: "Runtime actions",
    collapsedHint: "Collapsed. Expand to access Start QA Chromium, Verify current Incident, and Autofill current Incident.",
    statusReady: "Ready",
    statusBusy: "Working",
    statusBlocked: "Blocked",
    statusSuccess: "Verified",
    statusVerified: "Current form verified; Autofill remains limited to approved text fields.",
    statusCdpReady: "CDP ready; Verify enabled.",
    statusWaiting: "Waiting for sanitized CDP readiness from the dedicated QA Chromium profile.",
    startTitle: "Start QA Chromium",
    startDescription: "Opens the dedicated QA profile only; manual login remains required.",
    starting: "Starting QA Chromium",
    verifyTitle: "Verify current Incident",
    verifyDescription: "Checks that the visible page is the expected Incident form before any fill.",
    verifying: "Verifying current Incident",
    autofillTitle: "Autofill current Incident",
    autofillDescription: "Fills approved text fields only after verify. Human handles ServiceNow actions.",
    autofilling: "Autofilling current Incident",
    readyChip: "Ready",
    waitingChip: "Waiting",
    disabledProductionReason: "Disabled: Production is read-only in this workbench; choose QA Test Environment for Start, Verify, and Autofill.",
    disabledTargetReason: "Disabled: configure an allowed QA target in Settings first.",
    disabledBusyReason: "Disabled: another runtime action is still working.",
    startReadyReason: "Ready: opens dedicated QA Chromium; manual login only.",
    verifyCdpReason: "Disabled: Start QA Chromium and wait for CDP readiness first.",
    verifyReadyReason: "Ready: sanitized CDP readiness is available.",
    autofillVerifyReason: "Disabled: Verify current Incident first.",
    autofillReadyReason: "Ready: approved text fields only; human handling remains manual.",
  ...
  settings: {
    ...
    footerNote: "Settings apply locally in this window. Runtime safety gates are unchanged.",
    ...
    qaTestEnvironment: "QA Test Environment",
    ...
    urlDescriptionProduction: "Production target remains read-only; Start, Verify, and Autofill stay unavailable outside QA.",
    qaSubmitPolicy: "QA fill requires Start, Verify, and approved text-only autofill gating.",
...
const operatorWorkbenchTranslations = {
  ...
  "zh-CN": {
    ...
    environment: { qa: "QA 环境", production: "生产" },
    ...
    runtime: {
      eyebrow: "运行时栏",
      title: "运行操作",
      collapseRuntime: "折叠运行操作栏",
      expandRuntime: "展开运行操作栏",
      collapsedTitle: "运行操作",
      collapsedHint: "已折叠。展开后可使用启动 QA Chromium、验证当前 Incident、自动填入当前 Incident。",
      statusReady: "就绪",
      statusBusy: "处理中",
      statusBlocked: "已阻止",
      statusSuccess: "已验证",
      statusVerified: "当前表单已验证；自动填入仍仅限已批准文本字段。",
      statusCdpReady: "CDP 已就绪；可以验证。",
      statusWaiting: "等待专用 QA Chromium 配置返回已清理的 CDP 就绪状态。",
      startTitle: "启动 QA Chromium",
      startDescription: "仅打开专用 QA 配置；仍需手动登录。",
      starting: "正在启动 QA Chromium",
      verifyTitle: "验证当前 Incident",
      verifyDescription: "在任何填入前检查当前页面是否为预期 Incident 表单。",
      verifying: "正在验证当前 Incident",
      autofillTitle: "自动填入当前 Incident",
      autofillDescription: "验证后仅填入已批准文本字段；ServiceNow 操作仍由人工处理。",
      autofilling: "正在自动填入当前 Incident",
      readyChip: "就绪",
      waitingChip: "等待",
      disabledProductionReason: "禁用：生产环境在此工作台中保持只读；如需启动、验证、自动填入，请选择 QA 测试环境。",
      disabledTargetReason: "禁用：请先在设置中配置允许的 QA 目标。",
      disabledBusyReason: "禁用：另一个运行时操作仍在处理中。",
      startReadyReason: "就绪：打开专用 QA Chromium；仅手动登录。",
      verifyCdpReason: "禁用：请先启动 QA Chromium 并等待 CDP 就绪。",
      verifyReadyReason: "就绪：已获得清理后的 CDP 就绪状态。",
      autofillVerifyReason: "禁用：请先验证当前 Incident。",
      autofillReadyReason: "就绪：仅已批准文本字段；人工处理保持不变。",
      runtimeStatus: "运行状态",
      sanitizedMode: "已清理模式",
      cdpLabel: "CDP",
      cdpReady: "就绪",
      cdpWaiting: "等待中",
      sanitizedEvidence: "仅显示已清理的运行证据。",
      noEvidence: "暂无运行证据；仅显示已清理状态。",
      safetyTitle: "安全说明",
    ...
    settings: {
      ...
      qaUrl: "QA 目标",
      productionUrl: "生产目标",
      urlDescriptionQa: "用于受控测试的授权 QA 落地页；验证后会隐藏该值。",
      urlDescriptionProduction: "生产目标保持只读；启动、验证、自动填入仅在 QA 中可用。",
      qaSubmitPolicy: "QA 填入需要启动、验证和已批准的文本字段门禁。",
  ...
  "zh-TW": {
    ...
    environment: { qa: "QA 環境", production: "生產" },
    ...
    runtime: {
      eyebrow: "執行欄",
      title: "執行操作",
      collapseRuntime: "收合執行操作欄",
      expandRuntime: "展開執行操作欄",
      collapsedTitle: "執行操作",
      collapsedHint: "已收合。展開後可使用啟動 QA Chromium、驗證目前 Incident、自動填入目前 Incident。",
      statusReady: "就緒",
      statusBusy: "處理中",
      statusBlocked: "已阻止",
      statusSuccess: "已驗證",
      statusVerified: "目前表單已驗證；自動填入仍僅限已核准文字欄位。",
      statusCdpReady: "CDP 已就緒；可以驗證。",
      statusWaiting: "等待專用 QA Chromium 設定回報已清理的 CDP 就緒狀態。",
      startTitle: "啟動 QA Chromium",
      startDescription: "僅開啟專用 QA 設定；仍需手動登入。",
      starting: "正在啟動 QA Chromium",
      verifyTitle: "驗證目前 Incident",
      verifyDescription: "在任何填入前檢查目前頁面是否為預期 Incident 表單。",
      verifying: "正在驗證目前 Incident",
      autofillTitle: "自動填入目前 Incident",
      autofillDescription: "驗證後僅填入已核准文字欄位；ServiceNow 操作仍由人工處理。",
      autofilling: "正在自動填入目前 Incident",
      readyChip: "就緒",
      waitingChip: "等待",
      disabledProductionReason: "停用：生產環境在此工作臺中保持唯讀；如需啟動、驗證、自動填入，請選擇 QA 測試環境。",
      disabledTargetReason: "停用：請先在設定中設定允許的 QA 目標。",
      disabledBusyReason: "停用：另一個執行操作仍在處理中。",
      startReadyReason: "就緒：開啟專用 QA Chromium；僅手動登入。",
      verifyCdpReason: "停用：請先啟動 QA Chromium 並等待 CDP 就緒。",
      verifyReadyReason: "就緒：已取得清理後的 CDP 就緒狀態。",
      autofillVerifyReason: "停用：請先驗證目前 Incident。",
      autofillReadyReason: "就緒：僅已核准文字欄位；人工處理保持不變。",
      runtimeStatus: "執行狀態",
      sanitizedMode: "已清理模式",
      cdpLabel: "CDP",
      cdpReady: "就緒",
      cdpWaiting: "等待中",
      sanitizedEvidence: "僅顯示已清理的執行證據。",
      noEvidence: "尚無執行證據；僅顯示已清理狀態。",
      safetyTitle: "安全說明",
    ...
    settings: {
      ...
      qaUrl: "QA 目標",
      productionUrl: "生產目標",
      urlDescriptionQa: "用於受控測試的授權 QA 落地頁；驗證後會隱藏該值。",
      urlDescriptionProduction: "生產目標保持唯讀；啟動、驗證、自動填入僅在 QA 中可用。",
      qaSubmitPolicy: "QA 填入需要啟動、驗證和已核准文字欄位門檻。",
  ...
  "es-ES": {
    ...
    runtime: {
      eyebrow: "Riel de ejecución",
      title: "Acciones de ejecución",
      collapseRuntime: "Contraer riel de acciones de ejecución",
      expandRuntime: "Expandir riel de acciones de ejecución",
      collapsedTitle: "Acciones de ejecución",
      collapsedHint: "Contraído. Expande para acceder a Start QA Chromium, Verify current Incident y Autofill current Incident.",
      statusReady: "Listo",
      statusBusy: "Trabajando",
      statusBlocked: "Bloqueado",
      statusSuccess: "Verificado",
      statusVerified: "Formulario actual verificado; Autofill sigue limitado a campos de texto aprobados.",
      statusCdpReady: "CDP listo; Verify habilitado.",
      statusWaiting: "Esperando CDP depurado desde el perfil QA Chromium dedicado.",
      startTitle: "Start QA Chromium",
      startDescription: "Abre solo el perfil QA dedicado; el inicio de sesión sigue siendo manual.",
      starting: "Iniciando QA Chromium",
      verifyTitle: "Verify current Incident",
      verifyDescription: "Comprueba que la página visible sea el formulario Incident esperado antes de rellenar.",
      verifying: "Verificando Incident actual",
      autofillTitle: "Autofill current Incident",
      autofillDescription: "Rellena solo campos de texto aprobados después de verificar. El humano maneja ServiceNow.",
      autofilling: "Rellenando Incident actual",
      readyChip: "Listo",
      waitingChip: "Espera",
      disabledProductionReason: "Deshabilitado: Producción es de solo lectura en este banco; elige QA Test Environment para Start, Verify y Autofill.",
      disabledTargetReason: "Deshabilitado: configura primero un destino QA permitido en Configuración.",
      disabledBusyReason: "Deshabilitado: otra acción de ejecución sigue trabajando.",
      startReadyReason: "Listo: abre QA Chromium dedicado; inicio de sesión manual solamente.",
      verifyCdpReason: "Deshabilitado: primero inicia QA Chromium y espera CDP listo.",
      verifyReadyReason: "Listo: el estado CDP depurado está disponible.",
      autofillVerifyReason: "Deshabilitado: primero verifica el Incident actual.",
      autofillReadyReason: "Listo: solo campos de texto aprobados; manejo humano permanece manual.",
      runtimeStatus: "Estado de ejecución",
      sanitizedMode: "Modo depurado",
      cdpLabel: "CDP",
      cdpReady: "Listo",
      cdpWaiting: "Esperando",
      sanitizedEvidence: "Solo evidencia de ejecución depurada.",
      noEvidence: "Sin evidencia de ejecución; solo se muestra estado depurado.",
    ...
    settings: {
      ...
      footerNote: "La configuración se aplica localmente en esta ventana. Las puertas de seguridad de ejecución no cambian.",
      ...
      urlDescriptionProduction: "El destino de producción sigue siendo de solo lectura; Start, Verify y Autofill solo están disponibles en QA.",
      qaSubmitPolicy: "El rellenado QA requiere Start, Verify y puertas de autofill de texto aprobado.",
...
export function App({
  ...
  function clearEnvironmentUrlSettings() {
    ...
    resetOperatorBrowserReadiness("Verify is disabled until Start QA Chromium reports sanitized CDP readiness after settings are cleared.");
  }
  ...
  switch (page) {
    ...
    case "history":
      return {
        ...
        detailCards: [
          ...
          { title: "Runtime evidence", body: "Runtime rail remains the source of truth for Start, Verify, and Autofill readiness." }
        ],
        contextTitle: "Recent outcomes",
        contextItems: ["Show status labels, not raw URLs", "Keep fingerprints hidden", "Do not imply Save/Submit/Update/Close approval"],
    ...
    case "search":
      return {
        ...
        heroBody: "Use the left search control to inspect local sanitized workbench content. Runtime actions stay in the right rail.",
        stats: [
        ...
        detailCards: [
          ...
          { title: "Search safety", body: "Search never exposes stored credentials, cookies, CDP endpoints, or raw ServiceNow URLs in the main workbench." }
        ],
    ...
    <section className="operator-runtime-panel target-runtime-panel" aria-labelledby="runtime-rail-title">
      ...
      <div className="runtime-action-list" aria-label={workbenchCopy.runtime.title}>
        {actionCards.map((action) => (
          <article className={action.ready ? "runtime-action-card ready" : "runtime-action-card blocked"} key={action.step}>
            <div className="runtime-action-topline">
              <span className="runtime-step-number">{action.step}</span>
              <span className={action.ready ? "runtime-mini-chip ready" : "runtime-mini-chip blocked"}>
                {action.ready ? workbenchCopy.runtime.readyChip : workbenchCopy.runtime.waitingChip}
              </span>
            </div>
            <h3>{action.title}</h3>
