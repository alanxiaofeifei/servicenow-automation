...
export type QaIncidentDefaultPlannedField = {
  ...
  /**
   * True when a field is intentionally surfaced as a review step instead of an automatic default.
   * This preserves no-write behavior for controls whose safe value depends on the operator/page context.
   */
...
export type QaIncidentDefaultValuePlanSafety = {
  ...
  saveSubmitUpdateResolveCloseAllowed: false;
...
const qaAssignmentGroupName = "QA assignment group placeholder";

export const alanQaIncidentTestDefaults: QaIncidentTestDefaultProfile = {
  requester: "QA requester placeholder",
  category: "QA category placeholder",
  subcategory: "QA subcategory placeholder",
  location: "QA location placeholder",
  impact: "QA impact placeholder",
  urgency: "QA urgency placeholder",
  assignmentGroup: qaAssignmentGroupName,
  assignedTo: "QA assignee placeholder",
  routeOutState: "New",
  workNotesPrefix: "QA work notes"
};
...
export function buildQaIncidentDefaultValuePlan(
  ...
  const defaults = { ...alanQaIncidentTestDefaults, ...request.defaults };
...
export function applyQaWorkNotesPrefix(value: string, prefix = alanQaIncidentTestDefaults.workNotesPrefix): string {
...
function plannedFieldFor(
  ...
  const planned: QaIncidentDefaultPlannedField = {
    key,
  ...
  if (valueAndSource.source === "operator-confirmation-required") {
    planned.manualConfirmationRequired = true;
  }
  return planned;
}
...
function valueAndSourceFor(
  ...
  switch (key) {
    ...
    case "channel":
      return { value: "", source: "operator-confirmation-required" };
    case "impact":
...
function stopRules(): string[] {
  return [
    ...
    "Stop before Save, Submit, Update, Resolve, Close, attachment upload, outbound email, notification, ServiceNow API write, or bulk action.",
