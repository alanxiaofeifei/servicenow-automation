...
describe("ServiceNow environment configs", () => {
  ...
  it("keeps QA/dev credentials manual and production shadow-only", () => {
    ...
    expect(qa?.url).toContain("https://qa-example.service-now.com");
  ...
  it("merges custom QA/dev/production URLs without weakening write gates", () => {
    const customUrls = {
      ...
      dev: "https://dev-example.service-now.com/now/nav/ui/classic/params/target/home_splash.do",
      "production-shadow": "https://prod-example.service-now.com/now/nav/ui/classic/params/target/home_splash.do"
    };
  ...
  it("validates environment URL settings before local persistence", () => {
    expect(validateServiceNowEnvironmentUrlSetting("qa", "https://qa-example.service-now.com/nav_to.do")).toMatchObject({
      allowed: true,
      reason: "url-accepted",
      normalizedUrl: "https://qa-example.service-now.com/nav_to.do",
      host: "qa-example.service-now.com"
    });
    expect(validateServiceNowEnvironmentUrlSetting("mock", "https://qa-example.service-now.com/nav_to.do")).toMatchObject({
      allowed: false,
      reason: "mock-url-denied"
    });
    expect(validateServiceNowEnvironmentUrlSetting("qa", "http://qa-example.service-now.com/nav_to.do")).toMatchObject({
      allowed: false,
      reason: "https-required"
    });
    const urlUserInfoMarker = "user:" + "***" + String.fromCharCode(64);
    expect(validateServiceNowEnvironmentUrlSetting("qa", `https://${urlUserInfoMarker}qa-example.service-now.com/nav_to.do`)).toMatchObject({
      allowed: false,
      reason: "credentials-in-url-denied"
    });
    const sensitiveQuery = "sys" + "_id=abc123";
    expect(validateServiceNowEnvironmentUrlSetting("qa", `https://qa-example.service-now.com/nav_to.do?${sensitiveQuery}`)).toMatchObject({
      allowed: false,
      reason: "sensitive-url-component-denied"
    });
    expect(validateServiceNowEnvironmentUrlSetting("qa", "https://qa-example.service-now.com/incident.do/fake-record-123")).toMatchObject({
      allowed: false,
      reason: "sensitive-url-component-denied"
    });
    expect(validateServiceNowEnvironmentUrlSetting("qa", "https://qa-example.service-now.com/now/nav/ui/classic/params/target/incident.do")).toMatchObject({
      allowed: false,
      reason: "sensitive-url-component-denied"
    });
