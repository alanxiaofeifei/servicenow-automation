...
sda_desktop_preload_file() {
  if [ -f "$SDA_DESKTOP_DIR/out/preload/preload.mjs" ]; then
    printf '%s/out/preload/preload.mjs' "$SDA_DESKTOP_DIR"
    return 0
  fi

  if [ -f "$SDA_DESKTOP_DIR/out/preload/preload.cjs" ]; then
...
sda_desktop_preload_output_exists() {
  [ -f "$SDA_DESKTOP_DIR/out/preload/preload.mjs" ] || \
    [ -f "$SDA_DESKTOP_DIR/out/preload/preload.cjs" ]
}
...
sda_require_desktop_build_outputs() {
  ...
  if [ ! -f "$SDA_DESKTOP_DIR/out/main/main.js" ]; then
    echo "ERROR: Desktop build output is missing: $SDA_DESKTOP_DIR/out/main/main.js" >&2
    missing=1
  fi

  if ! sda_desktop_preload_output_exists; then
    echo "ERROR: Desktop build output is missing: $SDA_DESKTOP_DIR/out/preload/preload.mjs" >&2
    missing=1
  fi

  if [ ! -f "$SDA_DESKTOP_DIR/out/renderer/index.html" ]; then
    echo "ERROR: Desktop build output is missing: $SDA_DESKTOP_DIR/out/renderer/index.html" >&2
    missing=1
  fi
