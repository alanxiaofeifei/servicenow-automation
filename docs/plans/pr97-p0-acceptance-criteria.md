...
4. Browser connection status is visible in the app and is based on a real readiness check, not a launch attempt alone.
5. Check current ticket page is disabled before the browser connection is ready and enabled only after readiness is proven.
6. Page-check mode is read-only and performs no ServiceNow writes or field mutations.
...
### AC-04 — Browser connection status is visible and real

Accepted when:

- The app exposes a browser connection state such as Not started, Starting, Ready, Failed, or Disconnected.
- Ready is shown only after the dedicated test browser is actually reachable through the tool's internal browser-connection check, for example after DevToolsActivePort plus `/json/version` or an equivalent readiness probe succeeds.
- A mere process-start event or PowerShell success string is not enough to mark Browser Ready.
...
- If the browser closes or the browser connection becomes unreachable, the app returns to a non-ready state.

### AC-05 — Check current ticket page disabled/enabled transition

Accepted when:

- On fresh app startup, Check current ticket page is disabled.
- While QA Chromium is launching and the browser connection is not ready, Check current ticket page remains disabled.
- After browser connection readiness is proven, Check current ticket page becomes enabled.
- If browser connection readiness is lost, Check current ticket page becomes disabled again.
- This transition is visible to the operator and testable without reading source code.
- Check current ticket page must never become enabled merely because the app launched or because a browser process exists.

### AC-06 — Page check is read-only

Accepted when:

- Clicking Check current ticket page reads only the current browser page state needed to determine whether an Incident form is present and whether required fields are detectable.
- Page check performs no field writes, no keyboard input, no mouse clicks, no Save, no Submit, no Update, no Resolve, no Close, and no ServiceNow API writes.
- Page check does not alter Incident state, assignment, notes, category, requester, or any other form value.
- Page check returns a sanitized result such as checked/not checked, missing required fields, safety status, and next required operator action.
- If the current page is not a supported Incident form, Check current ticket page fails closed with a visible sanitized reason and performs no writes.

### AC-07 — Autofill remains separated from state-changing actions

Accepted when:

- Autofill is a separate explicit operator action after a successful page check, not part of Check current ticket page.
...
- The left column owns launch/connection state: Start QA Chromium, browser connection status, login guidance, and connection diagnostics.
...
5. Check-page button transition
   - Before Browser Ready, confirm Check current ticket page is disabled.
   - After Browser Ready, confirm Check current ticket page becomes enabled.
   - Close the dedicated browser or otherwise lose the browser connection, then confirm Check current ticket page disables again.
   - Fail if Check current ticket page is enabled before readiness or stays enabled after readiness is lost.

6. Page-check read-only behavior
   - Manually log in if needed and manually navigate to a QA/safe current Incident form.
   - Click Check current ticket page.
   - Expected: the app reports sanitized form/field readiness and next steps.
...
7. Autofill separation
   - Confirm Check current ticket page does not autofill.
...
- Test coverage includes the browser connection state machine, Check current ticket page disabled/enabled transitions, read-only page-check behavior, and no automated state-changing actions.
...
- Human observer confirms browser connection status is visible and real.
- Human observer confirms Check current ticket page transitions disabled to enabled only after browser readiness.
- Human observer confirms Check current ticket page performs no write.
...
- Page check is read-only by design and by test.
...
- Browser connection can be falsely marked Ready before the test browser is reachable.
- Page check can accidentally mutate the page if it shares code with autofill or action automation.
...
- `sna-browser-cdp`: Start QA Chromium, dedicated profile/window behavior, internal browser-connection readiness probe and state transitions.
...
5. Browser Ready appears before a real browser-connection readiness probe succeeds.
6. Check current ticket page is enabled before browser readiness or remains enabled after the browser connection is lost.
7. Page check changes any field, clicks anything, sends keyboard input, or performs any write.
8. Any code path automates Save, Submit, Update, Resolve, Close, or ServiceNow API writes.
9. Autofill is bundled with Check current ticket page or with a state-changing action.
