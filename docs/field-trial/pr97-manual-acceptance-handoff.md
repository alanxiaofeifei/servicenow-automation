...
This document is a sanitized handoff for external review and the next development plan. It intentionally omits real ServiceNow URLs, ticket identifiers, raw page-check identifiers, cookies, sessions, credentials, HARs, traces, screenshots, and private ticket content.
...
- The check-current-ticket step stayed disabled/greyed out because the dedicated browser connection was not started.
...
Important correction: local typecheck/build/test/privacy checks passed, but they did not prove the Windows double-click product flow, real QA Chromium launch, browser-connection readiness, or accepted UI redesign.

## What was completed in code before the failed manual acceptance

The prior implementation focused on safety hardening and local code checks:
...
- Desktop app includes a QA operator control path with intended numbered actions:
  - Start QA Chromium
  - Check current ticket page
  - Autofill allowed fields
- Internal implementation safety notes were added:
  - Autofill requires a prior read-only current-ticket check.
  - The app stores the current-ticket check state internally instead of asking the user to enter technical values.
  - App-to-runtime messages reject missing, malformed, or stale check state.
  - Runtime compares the checked page against the current page before fill.
  - Runtime/browser fill limits current execution to text fields only: Short description, Description, Work notes.
  - Runtime results avoid returning raw page-check values.
- Dedicated browser-connection helper hardening was added:
  - WSL exposure is limited to dev mode plus explicit confirmation.
  - QA/non-dev exposure should fail closed.
- Local checks passed before this manual failure was reported:
  - `pnpm typecheck`
  - `pnpm build`
  - `pnpm test`
  - `pnpm privacy:scan`
...
5. The app receives or displays a sanitized browser-connection status.
6. The check-current-ticket step becomes enabled only when the browser connection is ready.
...
- Electron renderer event handler path from button to the app's bridge.
- Electron main-process handler for browser launch.
...
- Whether the browser process spawned and immediately exited before it became ready.
- Whether the UI requires a browser connection but the launch result did not persist it in renderer state.

### P0 — Check current ticket remains unavailable

This is probably downstream of the browser-launch failure. Acceptance criteria should explicitly cover the disabled/enabled state transition:

- Before browser launch: check-current-ticket action disabled with clear reason.
- After successful launch and browser-connection readiness: check-current-ticket action enabled.
- If browser launch fails: check-current-ticket action disabled with a visible sanitized error and log path.
...
- Right column: runtime actions and controls, including browser launch, read-only ticket check, autofill buttons, templates, status, safety boundary, and environment controls.
...
- The read-only current-ticket check can read the current QA/dev Incident form structure without writing.
- Autofill must remain separated from Save/Submit/Update/Resolve/Close.
- Save/Submit/Update/Resolve/Close remain human-only unless a future separate reviewed approval path is explicitly implemented.

Near-term QA default profile, sanitized for this public handoff:

- This document intentionally does not publish private operational defaults for requester/person, category/subcategory, location, channel, assignment group, assigned-to handling, route-out state, or work-notes prefix.
- The private operator profile must be confirmed before any supervised QA/dev attempt.
- Required non-text fields remain manual-confirm or blocked until each field type has a separate safe implementation and privacy review.

Implementation note: the current executable Autofill path deliberately fills only the approved text fields: Short description, Description, and Work notes. Full required/starred field automation needs a separate control-type implementation and review because reference/select/status controls can have side effects.
...
3. Browser Connection Engineer
   - Owns dedicated Chromium launch, browser-connection readiness, internal readiness polling, sanitized launch errors, and read-only current-ticket connectivity.
   - Acceptance focus: Chromium visibly opens, the browser connection is usable, and the check-current-ticket action can run.

4. ServiceNow Form Safety Engineer
   - Owns read-only form inspection, field mapping, current-page freshness, text-only autofill, and later control-type slices.
   - Acceptance focus: no writes, no page-check leakage, correct required/default field plan.
...
6. QA Acceptance Tester
   ...
   - Separates local checks from real Windows UI acceptance.
   - Produces concise pass/fail evidence without sensitive artifacts.

7. Privacy/Safety Reviewer
   - Reviews every PR for ServiceNow URL/ticket/page-check/credential/session leakage and write-boundary regressions.
...
8. Release Integrator
   ...
   - Ensures code is pushed only after checks and acceptance notes are honest.

Suggested Kanban graph for the next phase:

- K1 Product acceptance reset: define P0 Windows launch + Chromium + read-only current-ticket acceptance criteria.
- K2 Runtime bug investigation: reproduce `Start QA Chromium` no-op from Windows double-click app and identify the broken button/helper/state path.
- K3 Runtime fix: repair button-to-browser launch and user-visible sanitized errors.
- K4 Current-ticket check fix: enable the check-current-ticket action after browser-connection readiness and prove read-only field inspection.
...
- K7 can run in parallel after K3 starts, but final verification still depends on the runtime/current-ticket path and the accepted UI not regressing.
...
2. Manual acceptance found Start QA Chromium no-op and the check-current-ticket action unavailable.
3. The UI redesign blocker has since been resolved by Alan's K6I5D2R manual acceptance.
4. Local automated checks passed but did not cover Windows product acceptance.
5. The next plan should recommend concrete Hermes Agent profiles and Kanban responsibilities before more coding.
6. The next coding phase should prioritize Windows launch and browser-connection observability before additional autofill scope.

## Non-goals until P0 is fixed

- Do not add Save/Submit/Update/Resolve/Close automation.
- Do not expand runtime autofill to reference/select/status fields before the browser launch and read-only current-ticket path work.
